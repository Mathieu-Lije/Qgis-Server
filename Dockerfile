...
ADD server/conf/qgis-server-nginx.conf /etc/nginx/nginx.conf
ADD server/start-xvfb-nginx.sh /usr/local/bin/start-xvfb-nginx.sh
RUN sed -i 's/\r//g' /usr/local/bin/start-xvfb-nginx.sh
